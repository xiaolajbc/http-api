import { ApiTabItem, EditStatus } from '@/components/ApiTab/ApiTab.type.ts'
import React, { createContext, useContext, useEffect, useState } from 'react'
import { initialActiveTabKey, initialTabItems } from '@/data/remote.ts'
...
interface MenuTabContextData {
  ...
  setActiveTabKey: React.Dispatch<
    React.SetStateAction<ApiTabItem['key'] | undefined>
  >
  lastActiveTabKey: ApiTabItem['key'] | undefined
  setLastActiveTabKey: React.Dispatch<
    React.SetStateAction<ApiTabItem['key'] | undefined>
  >
}
...
export function useMenuTabHelpers(): MenuTabHelpers {
  ...
  const addTabItem = useEvent<MenuTabHelpers['addTabItem']>(
    (payload, { autoActive = true, replaceTab } = {}) => {
      ...
      if (replaceTab) {
        setTabItems((items) =>
          items.map((it) => (it.key === replaceTab ? payload : it)),
        )
      } else {
  ...
  const removeTabItem = useEvent<MenuTabHelpers['removeTabItem']>((payload) => {
    setTabItems((items) => {
      ...
      if (activeTabKey === payload.key) {
        ...
        const valideTabKey =
          lastActiveTabKey &&
          newItems.findIndex((item) => item.key === lastActiveTabKey) !== -1
  ...
  const removeOtherTabItems = useEvent<MenuTabHelpers['removeOtherTabItems']>(
    () => {
      if (activeTabKey) {
        setTabItems((items) =>
          items.filter((item) => item.key === activeTabKey),
        )
      }
    },
  )
