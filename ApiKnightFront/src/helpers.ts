...
/** 判断两个菜单是否在同一分组。 */
export function isMenuSameGroup(
  left: ApiMenuData,
  right: ApiMenuData,
): boolean {
  switch (left.type) {
    ...
    case MenuItemType.ApiSchema:
      return (
        right.type === MenuItemType.ApiSchemaFolder ||
        right.type === MenuItemType.ApiSchema
      )

    case MenuItemType.RequestFolder:
    case MenuItemType.HttpRequest:
      return (
        right.type === MenuItemType.RequestFolder ||
        right.type === MenuItemType.HttpRequest
      )
...
export function findChildrenById(
  arr: ApiMenuData[],
  id: ApiMenuData['id'],
): ApiMenuData[] {
